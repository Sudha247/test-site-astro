...
We are gearing towards the release of a Minimum Stable Product for Dune Package
Management. For the uninitiated, [Dune Package
...
[Dune](https://github.com/ocaml/dune/). As we move towards releasing its Minimum
Stable Product, we are also in the process of battle-testing it in various
projects.

[OxCaml](https://oxcaml.org/) is a compiler branch developed by Jane Street with
fast-moving compiler extensions they would eventually like to upstream to the
mainstream compiler. Jane Street uses this compiler branch internally, and it is
being put to use in projects outside. Recently at ICFP 2025, there was a
[tutorial of OxCaml](https://github.com/oxcaml/tutorial-icfp25). This was
...
Management (`dune pkg`). Before I started, I was of the opinion that this was going to
be a straightforward change of a few lines, and ta da, everything would work (famous
last words...). But this process ended up being a series of interesting
findings, and I decided to write this blog post to summarise our findings.
...
processes, it was still running. I discussed this with
[@art-w](https://github.com/art-w), and he mentioned having come across the same
issue in the past. This was happening due to nested invocations of Dune, and
Dune failing to pick up concurrency. More details are
...
the Dune version of `ocamlbuild`, which is needed by `dune pkg`. There are some
patches on top of `ocamlbuild` in the Dune overlays. But it was picking up the
OxCaml version of `ocamlbuild`, presumably also having its own set of patches.
Now, this means we have two different forks of `ocamlbuild`. We need to combine
the patches into a single branch in order for this to work with both
OxCaml and `dune pkg`. Furthermore, I pretended this to be the OxCaml version, as
there were hard dependencies on the specific OxCaml version. But this pretend
version does build. It's still to be seen what happens upstream.
...
true at the time of writing this post, it may have changed if you're reading it
in the future.
